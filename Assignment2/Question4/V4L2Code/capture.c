...
int main(int argc, char **argv)
{
    ...
    open_device();
    init_device();
    retVal = clock_gettime(CLOCK_REALTIME, &startTime);
    start_capturing();
    mainloop();
    stop_capturing();
    retVal = clock_gettime(CLOCK_REALTIME, &endTime);

    delta_t(&endTime, &startTime, &diffTime);
    printf("Difference Seconds = %ld Milliseconds = %ld", diffTime.tv_sec , diffTime.tv_nsec / NSEC_PER_MSEC);
    int total_time_in_ms = (diffTime.tv_sec * 1000) + (diffTime.tv_nsec / NSEC_PER_MSEC);
    int average_time_in_ms = total_time_in_ms / frame_count;
